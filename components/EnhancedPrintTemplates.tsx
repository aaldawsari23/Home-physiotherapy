...
interface EnhancedPrintTemplateProps {
  ...
  staff?: Staff[];
  type: 'visit' | 'assessment' | 'patient-summary' | 'care-plan' | 'driver-route' | 'visit-history' | 'assessment-history';
  additionalData?: {
    visits?: Visit[];
...
const VisitHistoryPrint: React.FC<{ patient: Patient; visits: Visit[] }> = ({ patient, visits }) => (
  <div className="print-page">
    <PrintHeader 
      patient={patient} 
      title="Visit History Report" 
      subtitle="تقرير سجل الزيارات"
    />
    <PatientInfoCard patient={patient} />
    <div className="space-y-4">
      {visits.map(visit => (
        <div key={visit.id} className="print-no-break">
          <VisitPrint visit={visit} patient={patient} />
        </div>
      ))}
    </div>
  </div>
...
const AssessmentHistoryPrint: React.FC<{ patient: Patient; assessments: Assessment[] }> = ({ patient, assessments }) => (
  <div className="print-page">
    <PrintHeader 
      patient={patient} 
      title="Assessment History Report" 
      subtitle="تقرير سجل التقييمات"
    />
    <PatientInfoCard patient={patient} />
    <div className="space-y-4">
      {assessments.map(assessment => (
        <div key={assessment.id} className="print-no-break">
          {assessment.role === 'Doctor' && <DoctorAssessmentPrint assessment={assessment as DoctorAssessmentData} patient={patient} />}
          {assessment.role === 'Nurse' && <NurseAssessmentPrint assessment={assessment as NurseAssessmentData} patient={patient} />}
...
const EnhancedPrintTemplate: React.FC<EnhancedPrintTemplateProps> = ({ 
  ...
  switch (type) {
    ...
    case 'visit-history':
      return additionalData?.visits ? <VisitHistoryPrint patient={patient} visits={additionalData.visits} /> : null;

    case 'assessment-history':
      return additionalData?.assessments ? <AssessmentHistoryPrint patient={patient} assessments={additionalData.assessments} /> : null;
