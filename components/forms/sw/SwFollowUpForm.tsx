...
interface Props {
    ...
    onSave: (data: SwFollowUpData, noteText?: string) => void;
    onCancel: () => void;
}
...
const SwFollowUpForm: React.FC<Props> = ({ initialData, onSave, onCancel }) => {
    ...
    const handleSave = () => {
        ...
        const noteLines = [
            `Social Status: ${formData.socialStatus}`,
            `Updates: ${[...formData.updates].join(', ') || 'None'}`,
            `Interventions: ${[...formData.interventions].join(', ') || 'None'}`,
            `Short Term Plan: ${formData.shortTermPlan}`,
            `Escalation Indicators: ${[...formData.escalationIndicators].join(', ') || 'None'}`,
            `Escalation Paths: ${[...formData.escalationPaths].join(', ') || 'None'}`,
        ];
        if (formData.note) {
        ...
        const saveData: SwFollowUpData = {
            situationChange: formData.socialStatus,
            actionsTaken: Array.from(formData.interventions) as any[],
            plan: formData.shortTermPlan.includes('الاستمرار') ? 'الاستمرار' : 'تغيير',
            swNote: noteText || undefined
        };
        onSave(saveData, noteText);
    };
