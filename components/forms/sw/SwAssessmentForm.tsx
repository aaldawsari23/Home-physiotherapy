...
const SwAssessmentForm: React.FC<AssessmentFormProps> = ({ patient, onSave, onCancel }) => {
    ...
    const handleSave = (e: React.FormEvent) => {
        ...
        const noteLines = [
            ...
            `Primary Caregiver: ${formData.primaryCaregiver}`,
            `Housing: ${formData.housingType} - ${formData.housingSuitability}`,
            `Income: ${formData.incomeLevel} from ${formData.incomeSource}`,
            `Support Level: ${formData.supportLevel}`,
            `Psychological Status: ${[...formData.psychStatus].join(', ') || 'None'}`,
            `Social Challenges: ${[...formData.socialChallenges].join(', ') || 'None'}`,
            `External Resources: ${[...formData.externalResources].join(', ') || 'None'}`,
        ];
        if (formData.note) {
        ...
        const assessment: Assessment = {
            ...
            templateId: 'sw-assessment-v2',
            role: Role.SocialWorker,
            assessorId: currentUser.رقم_الهوية,
            assessorName: currentUser.الاسم,
            data: { ...formData, utilities: [...formData.utilities], psychStatus: [...formData.psychStatus], socialChallenges: [...formData.socialChallenges], externalResources: [...formData.externalResources] },
