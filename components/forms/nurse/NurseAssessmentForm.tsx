...
interface AssessmentFormProps {
    patient: Patient;
    onSave: (assessment: Assessment, noteText?: string) => void;
    onCancel: () => void;
}
...
const NurseAssessmentForm: React.FC<AssessmentFormProps> = ({ patient, onSave, onCancel }) => {
    ...
    const handleSave = (e: React.FormEvent) => {
        ...
        // Generate a descriptive note from the form data
        const noteLines = [
            `Vitals: BP ${formData.vitals.bp_systolic}/${formData.vitals.bp_diastolic}, HR ${formData.vitals.hr}, RR ${formData.vitals.rr}, Temp ${formData.vitals.temp}, SpO2 ${formData.vitals.spo2}`,
            `Pain: ${formData.vitals.pain}/10`,
            `Falls Risk: ${formData.falls_risk}`,
            `Pressure Risk: ${formData.pressure_risk}`,
            `Skin Status: ${formData.skin_status}`,
            `Interventions: ${[...formData.interventions].join(', ') || 'None'}`,
        ];
        if (formData.note) {
            noteLines.push(`Note: ${formData.note}`);
        }
        const noteText = noteLines.join('\n');

        const assessment: Assessment = {
            patientId: patient.id,
            id: '', // The repo will generate the ID
            templateId: 'nurse-assessment-v2',
            role: Role.Nurse,
            assessorId: currentUser.رقم_الهوية,
            assessorName: currentUser.الاسم,
            data: { ...formData, interventions: [...formData.interventions], education_topics: [...formData.education_topics], wound_details: formData.wound_details ? { ...formData.wound_details, locations: [...formData.wound_details.locations] } : undefined },
            createdAt: '' // The repo will set the date
        };

        // The onSave function should now handle the note text
        onSave(assessment, noteText);
    };
