...
interface Props {
    ...
    onSave: (data: PtFollowUpData, noteText?: string) => void;
    onCancel: () => void;
}
...
const PtFollowUpForm: React.FC<Props> = ({ initialData, onSave, onCancel }) => {
    ...
    const handleSave = () => {
        ...
        const noteLines = [
            `Patient Status: ${formData.patientStatus}`,
            `Interventions: ${[...formData.interventions].join(', ') || 'None'}`,
            `Modalities: ${[...formData.modalities].join(', ') || 'None'}`,
            `Equipment: ${[...formData.equipment].join(', ') || 'None'}`,
            `Tolerance: ${formData.tolerance}`,
            `Education: ${[...formData.education].join(', ') || 'None'}`,
            `Plan: ${formData.plan}`,
        ];
        if (formData.note) {
