...
const PtAssessmentForm: React.FC<AssessmentFormProps> = ({ patient, onSave, onCancel }) => {
    ...
    const handleSave = (e: React.FormEvent) => {
        ...
        const noteLines = [
            ...
            `Bed Mobility: ${formData.bedMobility}`,
            `Transfers: ${formData.transfers}`,
            `Ambulation: ${formData.ambulation}`,
            `Balance: ${formData.balance}`,
            `Problems: ${[...formData.problems].join(', ') || 'None'}`,
            `Short Term Goals: ${[...formData.shortTermGoals].join(', ') || 'None'}`,
            `Long Term Goals: ${[...formData.longTermGoals].join(', ') || 'None'}`,
        ];
        if (formData.note) {
        ...
        const assessment: Assessment = {
            ...
            templateId: 'pt-assessment-v2',
            role: Role.PhysicalTherapist,
            assessorId: currentUser.رقم_الهوية,
            assessorName: currentUser.الاسم,
            data: { ...formData, problems: [...formData.problems], shortTermGoals: [...formData.shortTermGoals], longTermGoals: [...formData.longTermGoals] },
