...
const DoctorAssessmentForm: React.FC<AssessmentFormProps> = ({ patient, onSave, onCancel }) => {
    ...
    const handleSave = (e: React.FormEvent) => {
        ...
        const noteLines = [
            ...
            `Active Diagnoses: ${[...formData.activeDiagnoses].join(', ') || 'None'}`,
            `Primary Assessment: ${formData.primaryAssessment}`,
            `Goal: ${formData.goal}`,
            `Action: ${formData.action}`,
            `Next Visit: ${formData.nextVisit === 'Custom' ? formData.customNextVisit : formData.nextVisit}`,
        ];
        if (formData.note) {
        ...
        const assessment: Assessment = {
            ...
            templateId: 'doctor-assessment-v2',
            role: Role.Doctor,
            assessorId: currentUser.رقم_الهوية,
            assessorName: currentUser.الاسم,
            data: { ...formData, activeDiagnoses: [...formData.activeDiagnoses], medications: [...formData.medications], labsImaging: [...formData.labsImaging], referrals: [...formData.referrals] },
