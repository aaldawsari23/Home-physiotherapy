...
export const repo = {
  ...
  // Assessments
  async addAssessment(a: Omit<Assessment, "id" | "createdAt">, noteText?: string) {
    const db = await loadDB();
    const rec: Assessment = { ...a, id: uid("a"), createdAt: new Date().toISOString() };
    db.assessments.push(rec);

    // Create a more descriptive note
    const text = noteText || `Assessment saved (template: ${a.templateId}).`;
